...
const enum_declaration_node_1 = require("../ast/enum-declaration-node");
...
const type_alias_declaration_node_1 = require("../ast/type-alias-declaration-node");
...
const createEnumNode = (symbolId, enumValues, runtimeEnums, context) => {
    if (runtimeEnums === 'pojo') {
        // For POJO enums, we need to generate both the POJO object and a type alias
        const runtimeSymbolId = `Runtime${(0, case_converter_1.toKyselyPascalCase)(symbolId)}`;
        const enumSymbol = {
            node: new enum_declaration_node_1.EnumDeclarationNode(runtimeSymbolId, enumValues, 'pojo', {
        ...
        const typeAliasName = (0, case_converter_1.toKyselyPascalCase)(cleanSymbolId);
        const typeAliasExpression = new raw_expression_node_1.RawExpressionNode(`(typeof ${enumSymbol.node.id.name})[keyof typeof ${enumSymbol.node.id.name}]`);
        const typeAliasNode = new type_alias_declaration_node_1.TypeAliasDeclarationNode(typeAliasName, typeAliasExpression);
        const typeAliasSymbol = {
            node: typeAliasNode,
            type: 'Definition',
        };
        const typeSymbolName = context.symbols.set(symbolId, typeAliasSymbol);
        return new identifier_node_1.IdentifierNode(typeSymbolName);
    }
    else {
        // Runtime enum
        const enumSymbol = {
            node: new enum_declaration_node_1.EnumDeclarationNode(symbolId, enumValues, 'runtime', {
                identifierStyle: runtimeEnums === 'screaming-snake-case'
        ...
        return new identifier_node_1.IdentifierNode(enumSymbol.node.id.name);
    }
};
...
const createEnumDefinitionNodes = (context) => {
...
const createDefinitionNodes = (context) => {
    ...
    for (const { name, symbol } of context.symbols.entries()) {
        ...
        const argument = symbol.node.type === 'TypeAliasDeclaration'
            ? symbol.node
            : new alias_declaration_node_1.AliasDeclarationNode(name, symbol.node);
